...
public class Client {
	...
	static int port = 0;//Puerto del Tracker
	static ByteBuffer buf;						// para el manejo de datos
	static int id = 0;								// id de cada cliente
	...
	public static void main(String args[]) {
		...
		try {
			...
			System.out.println("\n\t.:Menu de opciones:.\n" +
        			...
        			"\n[Subir <Nombre.extension>]: Subir archivo <Nombre y extension>." +
        			"\n[Descargar <#indice>]: <Digite [Descargar #indice] disponible en la red>." +
                    "\n[Salir]\n");
            ...
            while (sideListener.isAlive()) {
            	...
            	if (userInput.equals("Salir")){
                	...
                	if (num_files_on_tracker > page_number * Constants.PAGE_SIZE) {
                		...
                		System.out.println("Son todos los archivos.");
            		continue;
                	...
                	if (userInput.length() < 6){
                		System.out.print("<Especificar nombre y extension...>");
                		continue;
                	}
	...
	private static void uploadFile(String filepath, OutputStream out, 
		...
		if (exists == 0){
			System.out.println("- Peticion: " + name + " procesado correctamente.\n");
		} else {
	...
	private static void retrieveFiles(OutputStream out, InputStream in, int page) throws IOException, RuntimeException{
		...
		System.out.println("- Peticion: Obtenidos " + num_files +"/" +num_files_on_tracker+
		...
		for (int i=0; i < num_files; i++){
			...
			if (name.length() == 0)
				throw new RuntimeException(
						"Conexion - fallida! (nombre de archivo no se ha peticion ejecutada correctamente)");
			System.out.println("Indice | Nom. archivo | Tamanio | Peers\n" + (i+1) + "\t " +name +" \t"+file_size+"bytes   " +num_peers+".\n");
